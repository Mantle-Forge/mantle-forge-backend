// Commit 0 - 2026-01-15T09:54:25.348Z
// Commit 4 - 2026-01-15T09:54:25.784Z
// Commit 5 - 2026-01-15T09:54:25.880Z
// Commit 6 - 2026-01-15T09:54:25.977Z
// Commit 7 - 2026-01-15T09:54:26.075Z
// 1768471037556
// 1768471036955
// 1768471036798
// 1768471036653
...
// 1768471032687
// Updated 2026-01-15
// Commit 33 - 2026-01-15T09:54:29.002Z
// Commit 29 - 2026-01-15T09:54:28.560Z
// Commit 28 - 2026-01-15T09:54:28.396Z
// Commit 27 - 2026-01-15T09:54:28.291Z
// Commit 26 - 2026-01-15T09:54:28.194Z
// Commit 25 - 2026-01-15T09:54:28.101Z
// Commit 24 - 2026-01-15T09:54:28.005Z
// Commit 23 - 2026-01-15T09:54:27.914Z
// Commit 22 - 2026-01-15T09:54:27.785Z
// Commit 20 - 2026-01-15T09:54:27.580Z
// Commit 19 - 2026-01-15T09:54:27.479Z
// Commit 18 - 2026-01-15T09:54:27.374Z
// Commit 17 - 2026-01-15T09:54:27.247Z
// Commit 16 - 2026-01-15T09:54:27.155Z
// Commit 15 - 2026-01-15T09:54:27.048Z
// Commit 14 - 2026-01-15T09:54:26.939Z
// Commit 13 - 2026-01-15T09:54:26.820Z
// Commit 12 - 2026-01-15T09:54:26.694Z
// Commit 11 - 2026-01-15T09:54:26.555Z
// Commit 10 - 2026-01-15T09:54:26.380Z
// Commit 9 - 2026-01-15T09:54:26.264Z
// Commit 8 - 2026-01-15T09:54:26.171Z
...
// Helper function to send transactions with proper nonce and gas management
async function sendTransactionWithRetry(contract, methodName, args, maxRetries = 3) {
  const { wallet, provider } = getEthersSetup();
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
        // EIP-1559: Increase maxFeePerGas by 20% per retry attempt to handle replacement
        const multiplier = 120n + BigInt((attempt - 1) * 20); // 120% base, +20% per retry
        gasOptions.maxFeePerGas = (feeData.maxFeePerGas * multiplier) / 100n;
        gasOptions.maxPriorityFeePerGas = feeData.maxPriorityFeePerGas;
      } else if (feeData.gasPrice) {
        // Legacy: Increase gas price by 20% per retry attempt
        const multiplier = 120n + BigInt((attempt - 1) * 20); // 120% base, +20% per retry
        gasOptions.gasPrice = (feeData.gasPrice * multiplier) / 100n;
      }
      
      // Add nonce to prevent replacement issues
      gasOptions.nonce = nonce;
      
      console.log(`[Transaction] Attempt ${attempt}/${maxRetries} - Nonce: ${nonce}`);
      
      // Send transaction with gas options
      const tx = await contract[methodName](...args, gasOptions);
      ...
      // Check if it's a replacement fee error
      if (errorCode === 'REPLACEMENT_UNDERPRICED' || 
          errorMessage.includes('replacement') || 
          errorMessage.includes('underpriced')) {
        
        if (attempt < maxRetries) {
          console.log(`[Transaction] Replacement fee too low, retrying with higher gas (attempt ${attempt + 1}/${maxRetries})...`);
          // Wait a bit before retrying (exponential backoff)
          await new Promise(resolve => setTimeout(resolve, 2000 * attempt));
          continue;
        } else {
          console.error(`[Transaction] Failed after ${maxRetries} attempts: ${errorMessage}`);
          throw error;
        }
      } else {
...
async function startOrReloadAgent(agent, agentPath, branch_hash = null) {
  ...
  const logDir = process.env.PM2_LOG_DIR || path.join(process.cwd(), '.pm2', 'logs');
  if (!fs.existsSync(logDir)) {
    fs.mkdirSync(logDir, { recursive: true });
    console.log(`[PM2] Created log directory: ${logDir}`);
  }
  ...
  const env = await new Promise((resolve, reject) => {
    const secrets = {
      ...
      BACKEND_URL: process.env.BACKEND_URL || 'https://mantle-git-agent.onrender.com',
  ...
  // Use node with ts-node/register or use npm script
  // Check if ts-node exists locally, otherwise use node with require hook
  const tsNodeBin = path.join(agentPath, 'node_modules', '.bin', 'ts-node');
  const tsNodeModule = path.join(agentPath, 'node_modules', 'ts-node');
  
  // CRITICAL: env is the resolved secrets object (awaited on line 279), so env: env is correct
  // Configure PM2 to output logs to stdout so they appear in Render logs
  // logDir is already declared at the top of the function
  
  // Use npm script if package.json has start script, otherwise use node with ts-node/register
  let pm2App;
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    } else if (fs.existsSync(tsNodeBin) || fs.existsSync(tsNodeModule)) {
      // Use node with ts-node/register
      pm2App = {
        name: pm2Name,
        ...
        env: {
          ...env,
          FORCE_COLOR: '1',
          TS_NODE_PROJECT: path.join(agentPath, 'tsconfig.json'),
        },
        exec_mode: 'fork',
        interpreter: 'node',
        interpreter_args: `-r ${fs.existsSync(tsNodeModule) ? tsNodeModule : 'ts-node/register'}`,
        cwd: agentPath,
      };
    } else {
      // Fallback: try ts-node directly
      pm2App = {
        name: pm2Name,
        script: path.join(agentPath, 'agent.ts'),
        env: {
    ...
    // No package.json, try direct ts-node
    pm2App = {
      name: pm2Name,
      script: path.join(agentPath, 'agent.ts'),
      env: {
  ...
  // Add logging configuration to all variants
  Object.assign(pm2App, {
    // Configure logging - merge logs and use timestamps
    merge_logs: true,
    log_date_format: 'YYYY-MM-DD HH:mm:ss Z',
    // Store logs in a predictable location
    error_file: path.join(logDir, `${pm2Name}-error.log`),
    out_file: path.join(logDir, `${pm2Name}-out.log`),
    log_file: path.join(logDir, `${pm2Name}-combined.log`),
...
app.get('/auth/github', (req, res) => {
  if (!GITHUB_CLIENT_ID) {
    return res.status(500).json({ 
      ...
      manual_setup: 'https://mantle-git-agent.onrender.com/webhook/github/push'
    });
  }
      ...
      try {
          ...
          try {
            ...
            if (hooksError.response?.status === 403) {
              ...
              return res.send(`
                ...
                <p>Add webhook URL: <code>https://mantle-git-agent.onrender.com/webhook/github/push</code></p>
        ...
        if (errorStatus === 404) {
          ...
          return res.send(`
            ...
            <p>2. Add webhook URL: <code>https://mantle-git-agent.onrender.com/webhook/github/push</code></p>
        ...
        if (agent) {
          ...
          try {
            ...
            if (!agentAddress) {
              ...
              try {
                const tx = await sendTransactionWithRetry(factoryContract, 'registerAgent', [branch_hash]);
            ...
            console.log(`🔗 Explorer: https://sepolia.mantlescan.xyz/address/${agentAddress}`);
...
app.post('/api/agents/manual-trigger', async (req, res) => {
  ...
  try {
    ...
    db.get('SELECT * FROM agents WHERE branch_hash = ?', [branch_hash], async (err, agent) => {
      ...
      if (agent) {
        const explorerUrl = agent.agent_address ? `https://sepolia.mantlescan.xyz/address/${agent.agent_address}` : null;
        return res.json({ 
      ...
      try {
        ...
        const explorerUrl = `https://sepolia.mantlescan.xyz/address/${agentAddress}`;
        res.json({ 
          success: true, 
          agent_id: newAgentId, 
          agent_address: agentAddress,
      ...
      db.all(
        ...
        (metricsErr, metrics) => {
          ...
          const possibleLogPaths = [
            path.join(logDir, `${pm2Name}-out.log`),
            path.join(logDir, `${pm2Name}-combined.log`),
          ...
          // Combine PM2 logs with metrics logs, prioritizing PM2 logs
          const allLogs = pm2LogLines.length > 0 
            ? [...pm2LogLines, ...logLines] 
            : logLines;
          ...
          res.status(200).json({ 
            ...
            note: pm2LogLines.length > 0 
              ? `Showing PM2 console logs from ${logDir} (${pm2LogLines.length} lines found)` 
              : `PM2 logs not found. Checked: ${possibleLogPaths.join(', ')}. Showing decision metrics only.`
          });
        }
      );
    });
...
// Serve landing page
// Serve static files (logo, fonts, etc.)
...
async function recoverAgentsFromBlockchain() {
  try {
    ...
    const knownAgents = [
      { repo_url: 'https://github.com/Marshal-AM/gitAgent.git', branch_name: 'main' },
      { repo_url: 'https://github.com/Marshal-AM/gitAgent.git', branch_name: 'aggressive' },
      { repo_url: 'https://github.com/Marshal-AM/gitAgent.git', branch_name: 'moderate' },
      { repo_url: 'https://github.com/Marshal-AM/gitAgent.git', branch_name: 'test-branch' }
    ];
...
app.listen(PORT, async () => {
  console.log('🚀 MantleForge Backend Server Started');
